...
The **Security** page is TianshanOS’s security hub. It centralizes the following capabilities:
...
4. **HTTPS Certificate Management**: Configure the device’s mTLS certificates for mutual authentication.
...
1. Visit the device’s WebUI in your browser (e.g., `http://192.168.4.1`)
2. Log in with the admin account (default password: `rm01`; **strongly recommended to change it on first login**)
3. Click the **“Security”** link in the top navigation bar
...
- **Persistent storage**: Keys are stored securely in the device’s NVS (non-volatile storage) and survive reboots.
...
If the list shows “No deployed hosts”:

- You haven’t deployed any keys via the WebUI yet.
...
**Use Cases**:
- Adding the key manually to another server’s `authorized_keys`
...
5. Subsequent connections compare the server’s fingerprint with the stored value.
...
Fingerprints are SHA256 hashes of the SSH server’s host key, e.g.:
...
| **Key Authentication Failed** | Public key may have been removed—redeploy or verify `authorized_keys` |
...
Please refer to Task One, Section 2.4: “Revoking from Deployed Hosts” for guidance.
...
The “HTTPS Certificates” block configures the device’s mTLS credentials so that it can act as an HTTPS server and participate in mutual authentication with clients.
...
```
┌─────────────┐
│   Root CA   │ Trust anchor (stored offline)
│  20-year TTL│
└──────┬──────┘
       │ issues
       ▼
┌─────────────┐
│ Intermediate│ Online signing CA (e.g., step-ca)
│     CA      │
│  10-year TTL│
└──────┬──────┘
       │ issues
       ▼
┌─────────────┐
│ Device Cert │ This device’s identity
│ (this device)│ Stored in NVS
│  1-year TTL │
└─────────────┘
```
...
If the Known Host Fingerprints section always shows “No known hosts”:

1. **First-time use**: You haven’t connected to any SSH servers yet.
2. **Initialization bug**: The known host module didn’t initialize properly when the security service started.
...
- Refresh the page—the list should populate.

**Method 2: Check the initialization**
- If Method 1 fails, see the “Troubleshooting” document’s section on “Known host list initially empty.”
...
**Warning**: This removes every known host fingerprint—use with caution.
...
If a saved host’s fingerprint changes, TianshanOS shows a security warning before connecting.
...
• Server reinstallation or key regeneration
• IP address now points to a different server

Host: 192.168.1.100:22
Stored fingerprint: SHA256:OldFingerprint123...
Current fingerprint: SHA256:NewFingerprint456...

Recommendation: If you confirmed the server was rebuilt or the key was rotated, click “Update Host Key”
...
- Don’t click “Update Host Key” immediately. Verify first.
...
3. **Physical verification**:
   ...
   - Compare to the “Current fingerprint” shown in the warning.
...
1. **Obtain the official fingerprint**:
   - Ask the server’s administrator for the SHA256 fingerprint.
...
4. **Access control**:
   - “Hide Key ID” prevents low-privilege users from seeing sensitive identifiers.
   - “Allow export” flag enables fine-grained controls.
...
2. Read the warning dialog:
   ...
   After deletion:
   • mTLS will be unavailable
   • You must regenerate keys and reapply for certificates
...
- Without it, browsers or clients will show “Unable to verify certificate.”
...
**Why it matters**: A mismatched certificate can’t complete TLS handshakes and will break HTTPS.
...
If the device has a public domain, use Let’s Encrypt:

1. Use the ACME protocol for automation.
2. Complete domain validation (HTTP-01 or DNS-01).
3. Receive a free 90-day certificate.

**Note**: Let’s Encrypt is best for public-facing services; for internal networks, an enterprise CA is recommended.
...
When the certificate shows “Activated” or “Expiring soon,” the card expands to reveal:
...
#### Q1: “Storage full” when generating a key?

**Symptom**: Clicking “Generate” shows “NVS storage full” or “Maximum number of keys reached.”
...
Recommended cadence:
- 🗓️ Quarterly: Review key list, delete unused entries.
- 🗓️ Semi-annually: Audit deployed hosts; revoke unnecessary access.
- 🗓️ Annually: Rotate keys for critical servers.
...
**Security rationale**:
- Private keys are sensitive; they’re non-exportable by default.
...
1. Generate a key with **“Allow Private Key Export”** enabled.
...
---
#### Q13: “Certificate and private key do not match”?

**Symptom**: After pasting the certificate, the error shows “Certificate and private key do not match.”
...
**Related buttons**:
- “Install Certificate” and “Install CA” also depend on a generated key pair.
- Only “Generate Key Pair” and “Delete Credentials” are always active.
...
**Scenario**: You need to confirm a server’s fingerprint (first-time connection or mismatch).
...
**Symptom**: The list reads “No known hosts” even after you connected to SSH servers.

**Possible Causes**:

**Cause 1: You truly haven’t connected yet**
...
|- Behavior: You’ve connected and trusted hosts, yet the list stays empty until you refresh the page.
...
**Symptom**: You deployed a key, yet the table still shows “No deployed hosts.”
...
---
#### Q6: “Authentication failed” during deployment?

**Symptom**: After entering the password, the UI shows “Authentication failed” or “Wrong password.”
...
#### Q5: “Connection timeout” during deployment?

**Symptom**: The deployment process hangs and returns “Connection timeout.”
...
**Symptom**: Deploying or using an ECDSA key fails with authentication errors like “Authentication failed” or “Unsupported key type.”
...
**Pre-deletion Checklist**:
1. ✅ Confirm the key is not on critical servers.
2. ✅ Revoke or remove deployments.
3. ✅ Export the private key if a backup is needed.
4. ✅ Notify teammates who rely on the key.
...
**Renewal timeline**:
- 📅 **>30 days left**: Routine check; no action required.
- 📅 **30-15 days left**: Prepare renewal and contact the CA.
- 📅 **15-7 days left**: Execute the renewal process.
- 📅 **<7 days**: Treat as high priority.
...
**How to backup**:
1. Click “View Certificate” to copy the device certificate.
...
- 🗓️ Monthly: Check the status card.
- 🗓️ Quarterly: Review certificate details for anomalies.
- 🗓️ After major changes: Revalidate certificate integrity.
...
**Priority order**:
1. 🔴 Verify whether the server was rebuilt or keys changed.
2. 🟡 Investigate the network environment (untrusted Wi-Fi, proxies).
3. 🟢 Update the host key only after confirming legitimacy.
...
- 🚫 Projects finish.
- 🔄 Employees leave.
- 🔒 Servers are retired or moved.

**Action**:
1. Click “Revoke” from the Deployed Hosts list.
2. Or remove the entry from the server’s `authorized_keys`.

#### 4. Maintain a key rotation plan

**Strategy**:
- 🔄 Rotate critical server keys annually.
- 🔄 Rotate regular server keys every 2–3 years or as required.
- 🔄 Rotate test keys after the project ends.
...
**Recommendations**:
1. 📖 Read this guide fully before using the security page for the first time.
2. 🧪 Practice the workflows in a test environment first.
3. 🔖 Bookmark this page for quick reference.
4. 📚 Dive into the referenced technical docs for advanced topics.

**Safety reminders**:
- 🔐 Keep keys and certificates protected.
- 🚨 Treat all security warnings (e.g., fingerprint mismatches) seriously.
- 🔄 Regularly review and rotate keys/certificates.
- 📋 Log important changes and operations.
